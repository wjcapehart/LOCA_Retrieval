      ...
      do
         ...
         if [ ${PERIOD_STRING} ==  "1950-2005" ] || [ ${PERIOD_STRING} ==  "2006-2099" ]; then
            ...
            if [[ ${PAR} == "pr" ]]; then
               ...
               export OUTFILE=${CLIPPED_OUTDIR_A}/${DATASET}_${VARNAME}_${PERIOD_STRING}_CDO_YEARLY_DAILY_MAX.nc

               echo
               echo processing ${OUTFILE}

               echo
               echo cdo -b I32 -O  -z zip_8 yearmax  ${SUBSETFILE} ${OUTFILE}
               echo
                    cdo -b I32 -O  -z zip_8 yearmax  ${SUBSETFILE} ${OUTFILE}
               echo

               echo
               echo ncatted -h -O -a     long_name,${VARNAME},m,c,"Annual Maximum Daily Precipitation" ${OUTFILE}
               echo
                    ncatted -h -O -a     long_name,${VARNAME},m,c,"Annual Maximum Daily Precipitation" ${OUTFILE}
               echo

               echo
               echo ncatted -h -O -a   description,${VARNAME},m,c,"Annual Maximum Daily Precipitation" ${OUTFILE}
               echo
                    ncatted -h -O -a   description,${VARNAME},m,c,"Annual Maximum Daily Precipitation" ${OUTFILE}
               echo
